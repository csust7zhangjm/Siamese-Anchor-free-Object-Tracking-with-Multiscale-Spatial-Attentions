...
from myTracker.core.xcorr import xcorr_fast, xcorr_depthwise
...
class UPChannelMyTracker(myTracker):
    def __init__(self, feature_in=256, cls_out_channels=2):
        super(UPChannelMyTracker, self).__init__()
...
class MultiMyTracker(myTracker):
    def __init__(self, in_channels, cls_out_channels, weighted=False):
        super(MultiMyTracker, self).__init__()
        self.weighted = weighted
        for i in range(len(in_channels)):
            self.add_module('box'+str(i+2), DepthwiseMyTracker(in_channels[i], in_channels[i], cls_out_channels))
        if self.weighted:
